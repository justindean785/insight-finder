...
// providers (oathnet, indicia, exa, deepfind, hunter) keep first-timeout
// provider suppression. And minimax_web_search (Perplexity) is decoupled from the
// MiniMax reasoning tools so a search timeout can't suppress minimax_correlate.
...
Deno.test("oathnet_lookup timeout still suppresses the whole oathnet family (regression guard)", () => {
  const thread = "t-oath-timeout";
  clearThread(thread);

  recordResult(thread, "oathnet_lookup", "target@example.com", "default", { status: "timeout" });

  // First-timeout provider suppression is UNCHANGED for single-upstream paid providers.
  assertEquals(isProviderSuppressed(thread, "oathnet_lookup").suppressed, true, "oathnet suppresses on the first timeout");
  assertEquals(isProviderSuppressed(thread, "oathnet_stealer_search").suppressed, true, "a sibling oathnet endpoint is also suppressed");
  assertEquals(shouldRun(thread, "oathnet_victims_search", "target").allow, false, "the whole oathnet family is suppressed on first timeout");
