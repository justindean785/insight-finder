...
// Fix #5c — the Gemini-read arm of the dork relevance gate. env.ts captures keys
// at import, so set PERPLEXITY + GEMINI BEFORE the dynamic import (aaa_ pattern).
// A dork hit whose URL does NOT carry the seed is READ by Gemini; it is recorded
// only if the seed appears in the extracted text, else held as a candidate link.
