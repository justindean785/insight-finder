...
export const DEFAULT_TOOL_CATALOG: ToolRouteCandidate[] = [
  ...
  // ── Phase 1 free / no-required-key tools (fire on the matching artifact kind)
  {
    toolName: "crtsh_lookup",
    toolGroup: "certificate_transparency",
    supportsQueryTypes: ["domain", "url"],
    satisfiesPivotIds: ["domain_ssl_certs"],
    sourceClasses: ["infra_dns"],
    expectedValue: 42,
    providerAvailable: true,
    currentConcurrency: 0,
    maxConcurrency: 2,
  ...
  {
    toolName: "ransomwarelive_lookup",
    toolGroup: "malware_reputation",
    supportsQueryTypes: ["domain", "url"],
    satisfiesPivotIds: ["domain_reputation"],
  ...
  {
    toolName: "wayback_cdx_search",
    toolGroup: "archive",
    supportsQueryTypes: ["domain", "url"],
    satisfiesPivotIds: ["domain_archive"],
  ...
  {
    toolName: "census_geocode",
    toolGroup: "public_record",
    supportsQueryTypes: ["address", "property"],
    satisfiesPivotIds: ["exact_address_search"],
    sourceClasses: ["public_record"],
    expectedValue: 46,
