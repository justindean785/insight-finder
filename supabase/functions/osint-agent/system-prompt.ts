...
- PHONE → reverse-phone + business listing + breach + exact-phone dorks. EMAIL → rapidapi_breach_search FIRST, then breach_check; corroborate sparingly (≤1 call each, only on a hit worth a second corpus) with leakcheck_lookup / oathnet_lookup — both have scarce daily budgets (leakcheck 200/day, oathnet 100/day) and are NEVER the opening breach move. Then domain + local-part username reuse. (This applies to ANY email — the seed OR one discovered while working a phone/username/domain case.) USERNAME → cross-platform enumeration + profile + avatar + archive. DOMAIN/URL → WHOIS + DNS + certs + reputation + archive. IP → ASN + reverse/passive DNS + reputation.
