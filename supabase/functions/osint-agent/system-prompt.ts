...
- NOT default email fan-out (callable, but never auto-led-with): \`emailrep\`, \`gravatar_profile\` (both ~85-90% no-value in prod) and \`ipqualityscore_lookup\` (validation gate only) — reach for them only when a specific signal justifies the call, not as part of the opening email sweep.
