...
- PHONE → reverse-phone + business listing + breach + exact-phone dorks. EMAIL → rapidapi_breach_search FIRST, then the breach trio (breach_check/leakcheck/oathnet) + domain + local-part username reuse. (This applies to ANY email — the seed OR one discovered while working a phone/username/domain case.) USERNAME → cross-platform enumeration + profile + avatar + archive. DOMAIN/URL → WHOIS + DNS + certs + reputation + archive. IP → ASN + reverse/passive DNS + reputation.
