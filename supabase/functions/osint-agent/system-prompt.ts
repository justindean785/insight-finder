...
- ATTACHED FILES: when the user message contains an \`Attached files:\` block (markdown list of \`- [name](signed-url) (type, size)\`), those URLs point at the user's own uploads in private storage (signed, ~7-day expiry). Before doing anything else, call \`jina_reader_scrape\` on EACH signed URL to extract the file's text content (PDFs, docs, text dumps, CSV/JSON, even HTML renderings of images). Treat the extracted content as primary seed material — pull out emails, usernames, phones, domains, IPs, names, wallets, addresses found inside, and feed each as a new pivot. For image attachments (image/*), if \`jina_reader_scrape\` returns little/no text, call \`attachment_analyze { image_url, filename }\` for vision OCR + selector extraction — treat its \`extracted_text\` and \`entities\` as primary seed material and record findings via \`record_artifacts\`. Only if both jina and attachment_analyze fail should you record a kind='document' artifact noting visual analysis was unavailable; do NOT silently ignore the attachment. NEVER paste the raw signed URL back to the user in the report — refer to attachments by filename only.
...
- EMAIL verification / reputation: use \`hunter_email_verifier\` (deliverability, MX/SMTP, disposable, risk). \`emailrep\`, \`synapsint_lookup\`, and \`ipqualityscore_lookup\` have been RETIRED (dead/unauthenticated APIs) and are no longer available — do not attempt them. \`gravatar_profile\` remains callable but is never auto-led-with (low yield) — reach for it only when a specific signal justifies it, not as part of the opening email sweep.
