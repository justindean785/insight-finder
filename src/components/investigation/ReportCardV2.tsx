    ...
    <div className="border-b border-border-subtle px-6 py-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="font-mono text-micro uppercase tracking-[0.18em] text-muted-foreground/60">Verdict</span>
          <span
...
function Seed({ seed }: { seed: { value: string; type: string } }) {
  return (
    <div className="grid grid-cols-[140px_1fr_auto] items-center gap-6 border-b border-border-subtle px-6 py-4">
      <span className="font-mono text-micro uppercase tracking-[0.22em] text-muted-foreground/60">
              ...
              <div className="overflow-x-auto">
                ...
                <div className="grid grid-cols-[140px_1fr_180px_120px] gap-4 border-b border-border-subtle px-4 py-2">
                  {["Claim", "Value", "Source", "Conf"].map((h) => (
                    <span key={h} className="font-mono text-micro uppercase tracking-[0.16em] text-muted-foreground/60">
                      {h}
                    </span>
                  ))}
                </div>
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="mt-1 flex items-start gap-2 rounded-[2px] bg-surface-1 px-2.5 py-1.5">
                  <span className="font-mono text-micro uppercase tracking-[0.18em] text-muted-foreground/60">Needs</span>
      ...
      <div className="mt-5 overflow-x-auto rounded-[4px] border border-border-subtle bg-surface-2">
        ...
        <div className="grid grid-cols-[40px_70px_1fr_1fr_140px_56px] gap-3 border-b border-border-subtle bg-surface-1 px-4 py-2">
          {["ID", "Type", "Origin", "URL", "Retrieved", "Conf"].map((h) => (
            <span key={h} className="font-mono text-micro uppercase tracking-[0.18em] text-muted-foreground/60">{h}</span>
          ))}
        </div>
        {sources.map((s) => (
...
function CollapseBlock({
  ...
  return (
    ...
    >
      <span className="font-mono text-micro uppercase tracking-[0.22em] text-muted-foreground/60">{label}</span>
