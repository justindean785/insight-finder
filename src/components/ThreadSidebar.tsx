    ...
    return (
      <div className="w-14 h-full flex flex-col items-center py-3 gap-3">
        ...
        <button
          ...
          className={cn(
            "relative w-8 h-8 rounded-lg grid place-items-center text-accent glass-card hover:text-primary transition-colors",
  ...
  return (
    <div className="w-72 h-full flex flex-col">
      <div className="p-4 border-b border-border-subtle flex items-center gap-2">
        <div className="w-8 h-8 rounded-lg glass-strong grid place-items-center ring-1 ring-primary/25 shadow-[0_0_18px_-4px_hsl(var(--primary)/0.45)]">
          <SwarmMark className="w-5 h-5" />
        </div>
        <span className="font-display font-semibold tracking-tight text-base gradient-text">Swarmbot</span>
        <button
          onClick={onToggleCollapse}
      ...
      <div className="px-3 pt-3 pb-2">
        <Button
          onClick={newThread}
          size="sm"
          className="w-full justify-center gap-2 bg-gradient-to-r from-primary to-accent text-primary-foreground hover:opacity-90 shadow-[0_4px_20px_-6px_hsl(var(--primary)/0.6)] border-0"
        >
        ...
        <Link
          ...
          aria-label="Global Brain"
          className={cn(
            "relative mt-2 flex items-center gap-2 w-full px-3 py-2 rounded-md border text-[12px] font-medium uppercase tracking-[0.14em] transition-colors",
            onBrainRoute
              ? "border-primary/50 bg-primary/10 text-primary"
              : "border-border-subtle text-muted-foreground hover:text-foreground hover:border-primary/40 hover:bg-white/[0.03]",
          )}
        >
          <Brain className="w-4 h-4" strokeWidth={1.5} />
          <span>Brain</span>
      ...
      <div className="px-3 pb-2 flex items-center gap-2">
        <div className="flex-1 flex items-center gap-1.5 rounded-md glass-interactive px-2 py-1.5">
        ...
        <button
          onClick={() => setBrainOpen(true)}
          className={cn(
            "relative flex items-center gap-1 rounded-md border px-2 py-1.5 text-[11px] font-mono transition-colors hover:border-primary/60 hover:text-primary",
            memGrew
              ? "border-primary/60 bg-primary/15 text-primary animate-memory-ring"
              : "border-border/40 glass-card text-muted-foreground"
          )}
          title={`${memCount} cross-investigation memories — open learning log`}
        >
          <Brain key={memGrew?.key} className={cn("w-3 h-3", memGrew && "animate-brain-grow")} />
          <span>{memCount}</span>
          {memGrew && (
            <span
              key={`g-${memGrew.key}`}
      ...
      <div className="px-3 pb-2 flex items-center gap-1 overflow-x-auto scrollbar-none">
        {TYPES.map((t) => (
          <button
            ...
            className={cn(
              "shrink-0 px-2 py-0.5 rounded-full border text-[10px] font-mono uppercase tracking-wider transition-colors",
              typeFilter === t.key
                ? "border-primary/50 bg-primary/15 text-primary"
                : "border-border-subtle bg-transparent text-muted-foreground hover:text-foreground hover:bg-white/5"
            )}
          >
            {t.label}
          </button>
...
function ThreadRow({
  ...
  return (
    <Link
      to={`/chat/${t.id}`}
      className={cn(
        "group relative flex items-start justify-between gap-2 pl-3 pr-2 py-2 rounded-md text-sm transition-all hover:bg-white/5",
        active && "glass-interactive text-foreground ring-1 ring-primary/20",
