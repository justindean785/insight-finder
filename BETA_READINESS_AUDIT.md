...
> **▶ 2026-06-07 update (CI gate + tsc regressions):**
> - **`tsc` was NOT clean.** A cold `tsc --noEmit -p tsconfig.app.json` surfaced **3 frontend type errors** (`ThreadHeader.tsx` errorText `unknown`→`string`, `useThreadArtifacts.ts` Realtime payload cast, `intel.ts` `toolCallId` `unknown`→`string|null`). `vite build` does not typecheck, so these slipped past the build and the stale "tsc clean" claim. **All 3 fixed at the root** (widened `deriveToolTone` param; coerced `toolCallId`; cast Realtime payload through `unknown`).
> - **CI now exists.** Added `.github/workflows/ci.yml` (was none) — gates every PR + push-to-main on **lint · typecheck · test · build** (frontend job) and the **Deno edge suite** (edge job). Added a `typecheck` npm script. This closes the "PRs have no checks" gap from the prior session and stops type regressions from reaching the Lovable auto-deploy.

The platform is **functionally solid** — 177/177 vitest passing, tsc clean (after the 2026-06-07 fixes), the audit-driven `/health` probe is shipped, and most user-facing transport errors (401/403/404) are now mapped to explicit toasts in `ChatWindow.tsx`. However, **two structural issues block beta release**:
